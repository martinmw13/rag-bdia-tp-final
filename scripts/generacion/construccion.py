"""Construye en memoria el conjunto sintético reproducible.

Restricciones definidas en la especificación de datos sintéticos:
...
from .vectorial import distancia_coseno, fragmento_esperado, normalizar, vector_tematico
...
for documento_id, (codigo, titulo, clase_id, sensibilidad_id, procedencia, plan) in enumerate(
    ...
    documentos.append(Documento(documento_id, codigo, titulo, clase_id, sensibilidad_id, procedencia, True))
    ...
    for numero_version, estado, cantidad_secciones in plan:
        ...
        for posicion, (encabezado, texto) in enumerate(secciones, start=1):
            fragmentos.append(
                Fragmento(_id_fragmento, _id_version, posicion, encabezado, texto, None, clase_id, titulo)
            )
            _id_fragmento += 1
...
for fragmento_id, version_id, posicion, encabezado, texto, _, clase_id, _ in fragmentos:
    vector = vector_tematico(centroides[clase_id], ruido=0.12, rng=rng)
    vectores_por_fragmento[fragmento_id] = vector
    embeddings.append(
        Embedding(_id_embedding, fragmento_id, MODELO_ACTIVO, vector, INSTANTE_REFERENCIA - timedelta(days=30))
    )
    _id_embedding += 1
...
for fragmento_id in FRAGMENTOS_HISTORICOS:
    clase_id = fragmentos[fragmento_id - 1].clase_id
    embeddings.append(
        Embedding(
            _id_embedding,
            fragmento_id,
            MODELO_HISTORICO,
            vector_tematico(centroides[clase_id], ruido=0.30, rng=rng),
...
versiones_vigentes = {
    version.id for version in versiones
    if version.estado == "publicada"
    and version.vigente_desde is not None
    and version.vigente_desde <= INSTANTE_REFERENCIA
    and (version.vigente_hasta is None or version.vigente_hasta > INSTANTE_REFERENCIA)
}
fragmentos_recuperables = [
    fragmento.id
    for fragmento in fragmentos
    if fragmento.version_id in versiones_vigentes
]

clase_de_fragmento = {
    fragmento.id: fragmento.clase_id for fragmento in fragmentos
}
...
def fragmento_de(codigo_documento: str, numero_version: int, posicion: int) -> int:
    ...
    documento_id = next(
        documento.id for documento in documentos if documento.codigo == codigo_documento
    )
    version_id = next(
        version.id
        for version in versiones
        if version.documento_id == documento_id
        and version.numero_version == numero_version
    )
    return next(
        fragmento.id
        for fragmento in fragmentos
        if fragmento.version_id == version_id and fragmento.posicion == posicion
    )
...
esperado_vectorial = fragmento_esperado(
    vector_consulta_vectorial, fragmentos_recuperables, vectores_por_fragmento
)
esperado_hibrida_com = fragmento_esperado(
    vector_consulta_hibrida, recuperables_para(2), vectores_por_fragmento
)
...
ESCENARIOS = [
    Escenario("ESC-01", 3, "¿Qué condición comercial se aplica hoy al taladro percutor para el Corralón del Norte?", "exito", "estructurada"),
    Escenario("ESC-02", 1, "¿Qué pedidos y entregas requieren atención esta semana?", "exito", "estructurada"),
    Escenario("ESC-03", 3, "¿Cuál es el importe neto facturado por categoría de producto?", "exito", "estructurada"),
    Escenario("ESC-04", 3, "¿Cuáles son los productos principales de cada segmento de cliente?", "exito", "estructurada"),
    Escenario("ESC-05", 3, "¿Qué clientes no registran pedidos históricos?", "exito", "estructurada"),
    Escenario("ESC-06", 1, "¿Cómo debo proceder ante una entrega demorada?", "exito", "documental"),
    Escenario("ESC-07", 1, "¿Qué descuentos por volumen puedo ofrecer a un cliente mayorista?", "sin_fuente_autorizada", "negativa"),
]
...
for version in versiones:
    momento_carga = version.publicada_en or INSTANTE_REFERENCIA - timedelta(days=420)
    agregar_evento(5, 3, momento_carga - timedelta(days=1), "carga_borrador", "permitido",
                   "version_documental", version.id,
                   detalles={"documento": documentos[version.documento_id - 1].codigo,
                             "version": version.numero_version})
    if version.estado != "borrador":
        agregar_evento(5, 3, version.publicada_en, "publicacion", "permitido",
                       "version_documental", version.id,
                       detalles={"documento": documentos[version.documento_id - 1].codigo,
                                 "version": version.numero_version})
    if version.estado == "sustituida":
        agregar_evento(5, 3, version.vigente_hasta, "sustitucion", "permitido",
                       "version_documental", version.id,
                       detalles={"documento": documentos[version.documento_id - 1].codigo,
                                 "version": version.numero_version})
    if version.estado == "revocada":
        agregar_evento(5, 3, version.revocada_en, "revocacion", "permitido",
                       "version_documental", version.id,
                       detalles={"documento": documentos[version.documento_id - 1].codigo,
                                 "version": version.numero_version})
    ...
    if naturaleza == "estructurada":
        ...
        for ranking, fragmento_id in enumerate(ordenados, start=1):
            embedding_id = next(
                ...
                if embedding.fragmento_id == fragmento_id
                and embedding.modelo_id == MODELO_ACTIVO
            )
