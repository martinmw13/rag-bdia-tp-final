...
| Presentar una respuesta sin evidencia reconstruible | `evidencia_documental` referencia el `embedding_id` exacto, no el documento en general; una respuesta `exito` debe cerrar su transacción con al menos una evidencia (regla de negocio validada en el esquema). |
